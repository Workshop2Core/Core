...
public class MatchingController {
	...
	public void matching(){				
		List<OrderHistory> listBuy = orderHistoryService.listStatusOpenBuy();
		...
		for(OrderHistory orderListBuy : listBuy){ 
			...
			if(listSell != null){
				...
				BigDecimal saldoSeller = listSell.getUser().getSaldo().
						add(listSell.getPrice().
								multiply(listSell.getLot().
										multiply(perLot)));
				...
				BigDecimal saldoBuyer = orderListBuy.getUser().getSaldo().
						subtract(orderListBuy.getPrice().
								multiply(orderListBuy.getLot().
										multiply(perLot)));
				userBuyer.setSaldo(saldoBuyer);
				userService.update(idBuyer, userBuyer);
				
				//Update Transaksi Saham Seller
				TransaksiSaham transaksiSahamUpdateSeller = 
						transaksiSahamService.findDataSahamByUserIdAndIdSaham(idSeller, idSaham);
				transaksiSahamUpdateSeller.setLot(
						transaksiSahamUpdateSeller.getLot().subtract(listSell.getLot()));
				transaksiSahamService.update(
						transaksiSahamUpdateSeller.getIdtransaksi(), 
						transaksiSahamUpdateSeller);
